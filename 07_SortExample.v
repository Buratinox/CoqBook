...
Inductive Ordered : list nat -> Prop :=
  ...
  | Ord2 (n m : nat) (x : list nat) : n <= m -> Ordered (m :: x) -> Ordered (n :: m :: x).
...
Proof.
  intros n h t oht nh.
  induction t as [ | m t ]. simpl. reflexivity.
  destruct ( n == m ) as [ H1 | H2 ]; subst; simpl. 
  destruct ( m == m ) as [ H3 | H4 ]; subst; simpl.
  inversion oht; subst. apply (le_lt_trans h m h H1) in nh. 
...
Proof.
  intros x.
  induction x as [| hx tx ]. intros y epy oe oy.
  ...
  destruct (lt_eq_lt_dec hx hy) as [ [ H1 | H2 ] | H3 ].
    specialize (xy hx).
      assert (H5 : Count hx ty = 0). apply (Ord_count hx hy ty); assumption.
      assert (H6 : Count hx (hy :: ty) = 0). unfold Count.
        destruct ( hx == hy ) as [ H3 | H4 ]; subst. 
        apply lt_irrefl in H1. contradiction.  assumption.
      rewrite H6 in xy. simpl in xy.
      destruct ( hx == hx ) as [ H3 | H4 ]; subst. discriminate. now elim H4. subst.
    apply Perm_skip in xy. apply IHtx in xy. subst. reflexivity. 
    apply (OrderedTail hy tx oax). apply (OrderedTail hy ty oy). 
    specialize (xy hy).
      assert (H5 : Count hy tx = 0). apply (Ord_count hy hx tx); assumption.
      assert (H6 : Count hy (hx :: tx) = 0). unfold Count.
        destruct ( hy == hx ) as [ H1 | H2 ]; subst. 
        apply lt_irrefl in H3. contradiction.  assumption.
      rewrite H6 in xy. simpl in xy.
      destruct ( hy == hy ) as [ H1 | H2 ]; subst. discriminate. now elim H2.
Qed.
...
Lemma Perm_insert : 
  ...
  (n :: x) ~ (Insert n x).
Proof.
 induction x as [|n y H]; simpl. intro; reflexivity. 
 intros m; case (le_lt_dec m n); simpl. intro; reflexivity. 
...
Proof.
  ...
  elim H; simpl. 
    apply Ord1.
    intro m. case (le_lt_dec n m); simpl. 
      intro nm. apply Ord2; try assumption. apply Ord1.
      intro mn. apply Ord2; try assumption. firstorder.
      apply Ord1.
    intros n1 n2 y. case (le_lt_dec n n2); simpl; intros;
...
Proof.
  ...
  assert ( H1 : Insert n (Sort y) ~ Insert n y). 
    ...
    rewrite <- (Perm_insert (Sort y) n). apply Perm_skip. symmetry; auto.
...
Proof.
  destruct x0 as [ | m x ].
    specialize (p n). simpl in p. 
      destruct ( n == n ) as [ H1 | H2 ]. discriminate. now elim H2.
  split.
    symmetry. rewrite Perm_swap. apply Perm_skip. symmetry. assumption.
  intros ht. 
    assert ( ot : Ordered t). apply (OrderedTail h t ht). specialize (o ot).
    apply Ord2; try assumption. 
  destruct (le_lt_dec n m). apply (le_trans h n m); try assumption. auto with arith.
  destruct (le_lt_dec h m); try assumption. 
  assert (cmt : Count m t = 0). apply (Ord_count m h t); assumption.
  assert (cmnt : Count m (n :: t) = 0). unfold Count.
    destruct ( m == n ) as [ H3 | H4 ]; subst. 
    apply lt_irrefl in l. contradiction. assumption.
  specialize (p m). rewrite cmnt in p. simpl in p.
    destruct ( m == m ) as [ H3 | H4 ]; subst. discriminate. now elim H4. 
Defined.
...
Proof.
  ...
  elim H; simpl. 
    ...
    intro m. case (le_lt_dec n m); simpl. 
      ...
      intro mn. apply Ord2; try assumption. firstorder.
      apply Ord1.
    intros n1 n2 y. case (le_lt_dec n n1); simpl.
...
(* Function calculations and extraction. *)
